...
void TurnPreCalculation::_init(){
    auto turn_itr = TurnIterator(_int_molli_c, _shape_factor, _path_length, _target_ang, _v, _cp, _delta_t);
    float x = _start_x;
    float y = _start_y;
    float yaw = _start_ang;
    while(!turn_itr.isEnd()){
        float yawrate = turn_itr.getYawrate();
        float beta = turn_itr.getBeta();
        if(yawrate > 0.087f){ // 0.087rad = 5deg
            x += _v * cosf(yaw + beta) * sinf(yawrate * _delta_t * 0.5f) / (yawrate * 0.5f);
            y += _v * sinf(yaw + beta) * sinf(yawrate * _delta_t * 0.5f) / (yawrate * 0.5f);
        }                
        else{
            x += _v * cosf(yaw + beta) * _delta_t;
            y += _v * sinf(yaw + beta) * _delta_t;
        }
        yaw += yawrate * _delta_t;
        turn_itr.next();
    }
...
void TurnPreCalculation::_init180(){
    ...
    float path_length_min = 1.0f; 
    float path_length_max = 0.01f;
